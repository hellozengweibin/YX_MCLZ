  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
      ...
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="角色状态" clearable style="width: 180px" @change="handleQuery">
          <el-option v-for="dict in dict.type.sys_normal_disable" :key="dict.value" :label="dict.label" :value="dict.value"/>
        </el-select>
      </el-form-item>
      <el-form-item label="所属平台" prop="platformType" v-if="userID != 1 && platformOptions.length > 1">
        <el-select v-model="queryParams.platformType" placeholder="选择平台类型" :clearable="userID == 1" style="width: 200px" @change="handleQuery">
    ...
    <el-table v-loading="loading" :data="roleList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="角色名称" prop="roleName" show-overflow-tooltip width="220"/>
      <el-table-column prop="platformType" label="平台类型" width="170" align="center">
        <template slot-scope="scope">
          <el-select v-if="scope.row.roleId !== 1" size="small" v-model="scope.row.platformType"  @change="handleUpdatePlatform(scope.row)">
...
<!--      <el-table-column label="权限字符" prop="roleKey" show-overflow-tooltip width="150" />-->
      <el-table-column label="显示顺序" prop="roleSort" width="120" align="center">
        <template slot-scope="scope">
          <el-link type="primary" v-show="showSortUpdate==false" @click="showSortUpdate=true" :underline="false">{{scope.row.roleSort}}</el-link>
          <el-input-number v-show="showSortUpdate" v-model="scope.row.roleSort" size="small" @change="handleSortChange(scope.row)" :min="1" :max="100" style="width: 110px;" />
        </template>
      </el-table-column>
      ...
      <el-table-column label="账号数量" prop="userCount" width="110" align="center"/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" :close-on-click-modal="false" append-to-body>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="110px">
        <el-form-item label="所属平台" prop="platformType">
          <el-radio-group v-model="form.platformType" size="small" @change="handlePlatformTypeChange">
        ...
        <el-form-item prop="roleKey">
          <span slot="label">
            <el-tooltip content="全局唯一，不填可由后台自动生成" placement="top">
...
<script>
import {addRole, changeRoleStatus, dataScope, delRole, getRole, listRole, updateRole,updatePlatform} from "@/api/system/role";
...
export default {
  ...
  computed: {
    ...
    menuTreeData: function() {
      return this.menuOptions.filter(item => item.id == (0 - this.form.platformType))
    }
  },
  created() {
  ...
  methods: {
    ...
    handlePlatformTypeChange(type){
      const query = {platformType: type}
      this.getMenuTreeselect(query)
    },
    // 所有菜单节点数据
    ...
    handleSortChange(row){
      changeRoleStatus(row.roleId, row.status,row.roleSort).then(res => {
        if(res && res.code === 200) {
    ...
    handleUpdatePlatform(row){
      ...
      updatePlatform(param).then(res => {
        if(res && res.code === 200) {
          this.$modal.msgSuccess("平台类型修改成功");
          // this.$nextTick(() => {
          //   this.getList()
          // })
        } else {
    ...
    handleAuthUser: function (row) {
      ...
      this.$router.push({path:"/system/role-auth/user/" + roleId,query: {platform : row.platformType}});
    },
    /** 提交按钮 */
    submitForm: function () {
