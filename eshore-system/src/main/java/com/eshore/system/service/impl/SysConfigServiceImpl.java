...
import com.eshore.system.mapper.SysConfigMapper;
...
public class SysConfigServiceImpl implements ISysConfigService {
    ...
    @Override
    public <T> T getConfigByKeyWithType(String configKey, Supplier<T> supplier) {
        SysConfig config = new SysConfig();
        config.setConfigKey(configKey);
        SysConfig retConfig = configMapper.selectConfig(config);
        if (StringUtils.isNotNull(retConfig)) {
            redisCache.setCacheObject(getCacheKey(configKey), retConfig.getConfigValue());
            Integer valueType = retConfig.getValueType();
            ...
            switch (valueType) {
                ...
                case 2: // JSON
                    T t = supplier.get();
                    Object parse = JSON.parse(configValue);
                    if (t == null) return (T) BeanUtil.beanToMap(parse);
                    else BeanUtil.copyProperties(parse, t);
                    return t;
            }
        }
        return null;
